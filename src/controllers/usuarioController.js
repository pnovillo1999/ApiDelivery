...
controller.allusuarios = async function(req, res){	
	...
	const usuarios = await usuarioSchema.find().select('-password')
	if(usuarios){
...
controller.acceso = async function(req, res){	
	try{	
		...
		const acceso_usuario = await usuarioSchema.find({email:req.body.email, password:req.body.password}).select('-password')					

		if(acceso_usuario.length==0){
			return res.status(404).json({error:false, credenciales:"credenciales incorrectos"})
		}
...
controller.createUsuario = async function(req, res){
	...
	try{
		...
		return res.status(500).json({error:false, menssage:'hubor un error'})
	}	
}
